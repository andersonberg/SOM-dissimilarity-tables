...
def inicializacao(c, q, mapa_x, mapa_y, t_min, t_max, denom, matrizes, individuals_objects, pesos):
	...
	for objeto in individuals:
		criterios = [ (calcula_criterio(objeto, mapa, denom, matrizes, cluster.point, pesos), cluster) for cluster in mapa.flat ]
...
def calcula_criterio(obj, mapa, denom, matrizes, point1, pesos):
	
	sum_1 = sum([exp(-delta(point1, cluster.point) / denom) * sum(np.array(matrizes[:,int(obj.indice),int(cluster.prototipo.indice)]) * pesos) for cluster in mapa.flat])

	return sum_1

# Atualiza partição afetando cada indivíduo ao cluster mais adequado
def atualiza_particao(individuals, mapa, denom, matrizes, pesos):
	for objeto in individuals:
...
def calcula_prototipo(objeto_alvo, objetos, mapa, denom, matrizes, cluster, pesos):

	sum_1 = sum([exp(-delta(obj.cluster.point, cluster.point) / denom) * sum(np.array(matrizes[:,int(obj.indice),int(objeto_alvo.indice)]) * pesos ) for obj in objetos])
					
	return sum_1

# Seleciona o melhor protótipo para cada cluster
def atualiza_prototipo(mapa, individuals, denom, matrizes, q, pesos):
	for cluster in mapa.flat:
		if len(cluster.objetos) > 0:

			somas = [ (calcula_prototipo(obj, individuals, mapa, denom, matrizes, cluster, pesos), obj) for obj in individuals ]
...
def atualiza_pesos(objetos, mapa, denom, matrizes):
	...
	for j in range(len(matrizes)):
		...
		for matriz in matrizes:
			soma = sum( [ exp(-delta(objeto.cluster.point, cluster.point) / denom) * matriz[int(objeto.indice),int(cluster.prototipo.indice)] for objeto in objetos for cluster in mapa.flat ] )
			produto = produto * soma

		matriz_atual = matrizes[j]
		denominador = sum( [ exp(-delta(objeto.cluster.point, cluster.point) / denom) * matriz_atual[int(objeto.indice),int(cluster.prototipo.indice)] for objeto in objetos for cluster in mapa.flat ] )
...
#		text.extend([ str(peso) for peso in pesos ])
		...
		denom = 2. * pow(T,2)
		(mapa, prototipos, individuals) = inicializacao(c, q, mapa_x, mapa_y, t_min, t_max, denom, matrizes, individuals_objects, pesos)	
		...
		# while t < (n_iter - 1):
			...
			mapa = atualiza_prototipo(mapa, individuals, denom, matrizes, q, pesos)

			#Step 2: computation of the best weights
			pesos = atualiza_pesos(individuals, mapa, denom, matrizes)
...
#			text.extend([ str(peso)+" " for peso in pesos ])
			...
			mapa, individuals = atualiza_particao(individuals, mapa, denom, matrizes, pesos)
		...
		energia = calcula_energia(mapa, individuals, matrizes, T, pesos)
	...
	hoje = date.today()
	filename_result = nome_base + "-" + str(mapa_x) + "x" + str(mapa_y) + "-" + hoje.strftime("%d%m%y") + "_adapt_global_01.txt"
