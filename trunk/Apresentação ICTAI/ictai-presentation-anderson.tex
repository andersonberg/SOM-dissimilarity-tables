...
\begin{itemize}
\item Clustering methods organize a set of items into clusters 
\item Items within a given cluster have a high degree of similarity
\item Feature data and Relational data
\end{itemize}
\end{frame}
...
\end{center}
\begin{itemize}
\item $D_{\mbox{\boldmath$\lambda$\unboldmath$_l$}}$ is the global matching
\item relevance weight vector \boldmath$\lambda$\unboldmath$_l \, = (\lambda_{l1}, \ldots, \lambda_{lp})$ of the dissimilarity matrices \textbf{D}$_j$
\item Neighbourhood through kernel functions $K$
\end{itemize}
\end{frame}
...
0,1 & 0.99 & \textbf{1.05} & 0.55 & \textbf{1.09} & 0.27 & 0.69 & 0.79 \\ \hline
0,2 & \textbf{1.28} & 0.52 & 0.41 & 0.43 & \textbf{1.47} & 0.60 & \textbf{2.42} \\ \hline
