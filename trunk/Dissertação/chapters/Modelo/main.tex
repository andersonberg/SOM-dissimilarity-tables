...
O algoritmo SOM por lote para dados relacionais baseados em múltiplas tabelas de dissimilaridade deriva do SOM original por lote e do algoritmo de agrupamento para dados relacionais (\citep{DeCarvalho:2011}). Esta abordagem utiliza diferentes pesos para as matrizes de dissimilaridade, com o objetivo de ponderar a relevância de cada matriz na formação dos agrupamentos. Os pesos mudam a cada iteração do algoritmo, portanto não são definidos absolutamente, além disso, são diferentes de um agrupamento para outro.
...
%
%Muitos algoritmos foram adaptados para analisar dados relacionais. \cite{Kaufman:1987} apresenta uma adaptação do k-means para dados relacionais. Ainda, \cite{Golli:2004} apresenta um modelo de mapa auto-organizável por lote baseado em dados relacionais. 
