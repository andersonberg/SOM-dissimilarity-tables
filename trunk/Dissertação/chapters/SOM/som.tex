...
A rede SOM \citep{Kohonen2001} é uma ferramenta eficiente para visualização de dados em grandes dimensões. Em sua forma básica, ela produz uma grafo de similaridade dos dados de entrada. A rede SOM é um modelo de rede neural não-supervisionada de aprendizado competitivo, capaz de converter as relações estatísticas não-lineares entre os padrões de entrada multidimensionais em simples relações geométricas dos respectivos neurônios da rede, que se encontram dispostos em, geralmente, uma grade de nós bidimensionais, chamado de mapa. Como o SOM comprime os dados ao mesmo tempo em que preserva as relações topológicas e/ou métricas mais importantes, pode-se considerar que ele produz algum tipo de abstração. Portanto, visualização e abstração são as principais propriedades das redes SOM.

A rede SOM é capaz de realizar quantização de vetores e/ou agrupamento de dados preservando o ordenamento espacial dos dados. A preservação topológica dos dados é possível através da ordenação dos vetores protótipos (também chamados de centros, centróides ou vetores de referência) em um espaço de uma ou duas dimensões. Os vetores protótipos estão presentes nos neurônios da rede e possuem mesma dimensionalidade dos padrões de entrada. Durante o treinamento, os neurônios da rede procuram se adequar aos dados de entrada. No final, padrões de entrada próximos no espaço de entrada serão mapeados em neurônios semelhantemente próximos no mapa, preservando a topologia dos dados.

Formalmente, o mapa é descrito por um grafo ($C$ , \textit{$\Gamma$}). $C$ é um conjunto de $m$ neurônios interconectados por uma topologia discreta definida por \textit{$\Gamma$}. Para cada par de neurônios ($c, r$) no mapa, $\delta (c, r)$ é definida como a função de distância entre $c$ e $r$ no grafo. Esta distância impõe uma relação de vizinhança entre os neurônios. Cada neurônio $c$ é representado por um protótipo $p$-dimensional $\textbf{w}_c = (w_c^1, ..., w_c^p)$, onde $p$ é igual à dimensão dos vetores dos dados de entrada.
