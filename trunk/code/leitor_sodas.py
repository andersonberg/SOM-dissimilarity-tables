		...
		if title_base == "chevaux":
			...
			for line in range(0, len(lines), int(variavel_nominal.id)):
				if line > 0:
					...
					if not classes.has_key(classe_id):
						classes[classe_id] = Classe(classe_id, classe_id)
					classes[classe_id].inserir_objeto(individuals_objects[j])
					individuals_objects[j].classe_a_priori = classes[classe_id]
#					for cls in classes:
#						if cls.id == classe_id:
...
#						cls.inserir_objeto(individuals_objects[j])
	...
	for classe in classes.values():
		if not classe.objetos:
			del classes[classe.indice]
